...
public class ProcessingEmployees {
	...
	public static void main(String[] args) {
		...
		// Filter the salary with a specified range.
		Predicate<Employee> fourToSixThousand = m -> (m.getSalary()>=4000 && m.getSalary()<=6000);
		...
		// or : list.stream().filter( m -> (m.getSalary()>=4000 && m.getSalary()<=6000) )
		...
		list.stream().sorted(lastThenFirst).map(e->e.getName()).forEach(System.out::println);
		
		// Group employees by department
		System.out.printf("%nEmployees by department:%n");
		Map< String, List<Employee> > groupedByDepartment = list.stream().collect(Collectors.groupingBy(e->e.getDepartment()));
		...
		System.out.printf("Sum of Employees' salaries (via reduce method): %.2f%n",
						  list.stream().mapToDouble(e -> e.getSalary()).reduce(0, (val1, val2) -> val1 + val2));
		
		// average of Employee salaries with DoubleStream average method
		System.out.printf("Average of Employees' salaries: %.2f%n",
						  list.stream().mapToDouble(e -> e.getSalary()).average().getAsDouble());
